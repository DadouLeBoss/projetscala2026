...
object Main extends App {
  def apply(): Behavior[Unit] = Behaviors.setup { context =>
    ...
    val tankLourd = context.spawn(Actors.Tank("tankLourd", 500, 0), "tankLourd")
    val tankLeger = context.spawn(Actors.Tank("tankLeger", 500, 0), "tankLeger")
    val tankGas   = context.spawn(Actors.Tank("tankGas", 500, 0), "tankGas")
